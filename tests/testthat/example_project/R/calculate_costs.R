...
calculate_costs <- function(Markov_trace_,
  ...
  for(x in c("discounting_weights_", "Markov_trace_", "costs_")) {
  ...
  for(x in c("costs_", "discounting_weights_")) {
    assertthat::assert_that(
      assertthat::noNA(get(x)),
      msg = paste(
        "The object passed to the",
  ...
  # confirm all value(s) are 1 or less
  assertthat::assert_that(
    all(costs_ >= 0),
    msg = paste(
      "The object passed to the costs_ argument contains one or more negative",
      "value(s)"
    )
  )
  ...
  assertthat::assert_that(
    ...
    msg = paste(
      "The number of values in the vector passed to the costs_ argument is",
